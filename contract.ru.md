...
С помощью данной конструкции можно создавать условные выражения без создания `switch` определений.
...
Если ключ `definition` начинается с явного определения переменной за которым следует конструкция: `switch <expression>`,
