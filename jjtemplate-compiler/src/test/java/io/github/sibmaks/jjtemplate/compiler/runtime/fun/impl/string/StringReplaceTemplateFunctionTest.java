...
class StringReplaceTemplateFunctionTest {
    @InjectMocks
    private StringReplaceTemplateFunction function;

    static Stream<Arguments> replaceCases() {
        return Stream.of(
                Arguments.of("pello", "p", "h", "hello"),
                Arguments.of("pello", "[p]+", "h", "pello"),
                Arguments.of("", "p", "h", "")
        );
    }
    ...
    void directInvokeNotEnoughArgs() {
        ...
        assertEquals("string:replace: 3 arguments required", ex.getMessage());
    }
    ...
    void pipeInvokeWrongArgsCount() {
        ...
        assertEquals("string:replace: 2 arguments required", ex.getMessage());
    }
}
